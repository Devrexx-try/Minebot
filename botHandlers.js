...
function createBot() {
  ...
  bot.on('login', () => {
    console.log(`${bot.username} joined the server.`); // Corrected to use backticks
    afkBypass(bot);
    handleAutoAuth(bot);
    simulateLag(bot);
  });
  ...
  bot.on('end', () => {
    console.log(`${bot.username} disconnected. Attempting to reconnect...`); // Corrected to use backticks
    setTimeout(createBot, 5000);
  });

  bot.on('error', (err) => console.log(`${bot.username} error: ${err.message}`)); // Corrected to use backticks
  bot.on('kicked', (reason) => {
    console.log(`${bot.username} was kicked from the server: ${reason}`); // Corrected to use backticks
  });
